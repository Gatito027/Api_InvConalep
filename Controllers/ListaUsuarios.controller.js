...
const listaUsuarios = async (req, res) => {
    ...
    try {
        ...
        const data = await db.query("select * from obtener_usuarios();");
        logger.info(`Consulta de los usuarios realizada por ${userData.data.usuario}`);
        if (!data){
            response.isSuccess = false;
            response.message = "Sin usuarios";
            response.data = null;
        }
