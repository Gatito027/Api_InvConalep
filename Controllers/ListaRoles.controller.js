...
const listaRoles = async (req, res) => {
    ...
    try {
        ...
        const data = await db.query("select * from Obtener_Roles_con_Permisos();");
        logger.info(`Consulta de los roles realizada por ${userData.data.usuario}`);
        if (!data){
            response.isSuccess = false;
            response.message = "Sin roles";
            response.data = null;
        }
