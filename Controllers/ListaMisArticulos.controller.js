...
const ListaMisArticulos = async (req, res) => {
    const response = new ResponseDto();
    try {
        const {token} = req.cookies;
        const userData = AuthenticationExtensions.addJwtAuthentication(token);
        const data = await db.query("select * from obtener_articulos_por_usuario($1);", [userData?.data?.sub]);
...
module.exports = { ListaMisArticulos };